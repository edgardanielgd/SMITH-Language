...
// Probabilistic builtin functions
        ...
        try {
            ...
            for (int i = 0; i < arguments.size(); i++) {
                if (parameterTypes[i].equals(int.class)) {
                    if (arguments.get(i).type != Variable.INT) {
                        Error.throwError(
                            functionName + " expects an integer as argument " + (i + 1) + " but " +
                                typeToString( arguments.get(i).type ) +
                                " was given",
                            args
                        );
                        return -2;
                    ...
                    if (
                            arguments.get(i).type != Variable.FLOAT &&
                            arguments.get(i).type != Variable.INT
                    ) {
                        Error.throwError(
                                functionName + " expects a number as argument " + (i + 1) + " but " +
                                        typeToString( arguments.get(i).type ) +
                                        " was given",
                                args
                        );
            ...
            if (parameterTypes.length != arguments.size()) {
                Error.throwError(
                        functionName + " expects " + parameterTypes.length + " arguments but " +
            ...
            Error.throwError(
                    "Error while calling built-in function " + name + ": " + e.getMessage(),
                    args
            );
            return -2;
        }
